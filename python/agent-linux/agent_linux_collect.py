...
def clamav_enabled() -> bool:
    # FIX: déplacé après run_cmd, et amélioration de la détection du service
    try:
        ...
        if run_cmd(["which", "clamscan"]):
            return True
        # Vérifie aussi via systemctl
        status = run_cmd(["systemctl", "is-active", "clamav-daemon"])
...
def get_network_logs(limit: int = 20) -> List[str]:
    ...
    # FIX: filtrage sur les unités auth pertinentes uniquement
    journal = run_cmd([
        "bash", "-lc",
        f"command -v journalctl >/dev/null 2>&1 && "
        f"journalctl -n {limit} --no-pager -q -u sshd -u ssh -u sudo -u su"
    ])
    if journal:
...
def build_payloads(backend_url: str) -> Dict[str, Dict[str, Any]]:
    ...
    security_payload = {
        ...
        # FIX: champs Windows mis à None (non applicable) au lieu de False
        # pour éviter que le backend interprète False comme "poste hors ligne"
        "defender_enabled": None,
        "realtime_protection_enabled": None,
        "bitlocker_enabled": None,
        "pending_reboot": pending_reboot(),
        "clamav_enabled": clamav_enabled(),
        # FIX: ajout du timestamp pour que le backend sache que le poste est actif
...
def collect_auth_events_linux(limit: int = 50) -> List[Dict[str, Any]]:
    """
    FIX: Collecte ciblée sur les unités auth (sshd, sudo, su, pam, gdm).
    FIX: Regex améliorée pour extraire l'utilisateur dans les formats réels de logs.
    FIX: Ajout du timestamp et de l'IP source dans chaque event.
    FIX: limit augmenté à 50 pour ne pas manquer d'events entre deux cycles.
    """
    events: List[Dict[str, Any]] = []
    lines: List[str] = []

    # Journalctl avec filtre sur les unités et mots-clés auth
    journal = run_cmd([
        "bash", "-lc",
        f"command -v journalctl >/dev/null 2>&1 && "
        f"journalctl -n {limit} --no-pager -q "
        f"-u sshd -u ssh -u sudo -u su -u gdm -u lightdm "
        f"--since '5 minutes ago'"
    ])
    if journal:
        lines = [l for l in journal.splitlines() if l.strip()]

    # Fallback auth.log / secure
    if not lines:
    ...
    for line in lines:
        ...
        # Détection des types d'événements (ordre important : du plus spécifique au plus général)
        if re.search(r"session opened for user", line, re.IGNORECASE):
            evt_type = "auth.login"
        elif re.search(r"Accepted (password|publickey|keyboard-interactive)", line, re.IGNORECASE):
            evt_type = "auth.login"
        elif re.search(r"authentication failure|Failed password|Invalid user|FAILED LOGIN", line, re.IGNORECASE):
            evt_type = "auth.login_failed"
        elif re.search(r"account locked|user locked|maximum authentication attempts", line, re.IGNORECASE):
            evt_type = "auth.lockout"
        elif re.search(r"sudo:.*COMMAND=", line):
            evt_type = "auth.sudo"
        elif re.search(r"session closed for user", line, re.IGNORECASE):
            evt_type = "auth.logout"

        if not evt_type:
            continue

        # FIX: extraction username — couvre les formats sshd, pam, sudo, su
        user = ""
        for pattern in [
            r"for user[= ](\S+)",           # pam: "session opened for user alice"
            r"Accepted \S+ for (\S+)",       # sshd: "Accepted publickey for alice"
            r"Failed \S+ for (?:invalid user )?(\S+)",  # sshd: "Failed password for bob"
            r"Invalid user (\S+)",           # sshd: "Invalid user eve"
            r"sudo:\s+(\S+)\s+:",            # sudo: "sudo:  alice :"
            r"user=(\S+)",                   # format générique "user=alice"
            r"for (\S+) from",               # sshd générique
        ]:
        ...
        # Extraction IP source si présente
        src_ip = ""
        m_ip = re.search(r"from ([\d.]{7,15}|[0-9a-f:]{3,39})", line, re.IGNORECASE)
        if m_ip:
            src_ip = m_ip.group(1)
        ...
        events.append({
            ...
            "os": platform.system(),
            "source_ip": src_ip,
            "raw": line[:300],  # garder la ligne brute pour debug (tronquée)
...
def run_cycle(backend_url: str, verbose: bool = False) -> bool:
    ...
    # Envoi événements d'authentification vers le SIEM
    auth_events = collect_auth_events_linux()
    if verbose:
        print(f"[auth_events] {len(auth_events)} événement(s) collecté(s)")
    ...
    for evt in auth_events:
        try:
            ...
            if verbose:
                ...
                print(f"[auth_event] {evt['event_type']} user={evt.get('user', '?')} : {status}")
        except Exception as e:
            print(f"[auth_event] Erreur envoi : {e}")
